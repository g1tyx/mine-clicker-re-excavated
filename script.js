...
function loadOre(x) {
	...
	if (x<79) {
		...
		if (oreValueType==1) {
			document.getElementById("currentOreValue").innerHTML = "$" + format(Math.min(oreValues[currentOre-1], oreValues[currentOre-1]/(oreHitPoints[currentOre-1]/(game.damage - oreHardnesses[currentOre-1]))))
		}
		else {
		...
		if (oreValueType==1) {
			document.getElementById("currentOreValue").innerHTML = "$" + format(Math.min((1.8**(x-78)*8e17), (1.8**(x-78)*8e17)/((1.5**(x-78)*2.5e14)/(game.damage - (1.5**(x-78)*5e12)))))
		}
		else {
...
function changeOreValueType() {
	oreValueType = 1-oreValueType
	if (oreValueType==1) {
		document.getElementById("oreValueType").innerHTML = "Value/click"
		if (currentOre<79) {
			document.getElementById("currentOreValue").innerHTML = "$" + format(Math.min(oreValues[currentOre-1], oreValues[currentOre-1]/(oreHitPoints[currentOre-1]/(game.damage - oreHardnesses[currentOre-1]))))
		}
		else {
			document.getElementById("currentOreValue").innerHTML = "$" + format(Math.min((1.8**(currentOre-78)*8e17), (1.8**(currentOre-78)*8e17)/((1.5**(currentOre-78)*2.5e14)/(game.damage - (1.5**(currentOre-78)*5e12)))))
		}
	}
	else {
		document.getElementById("oreValueType").innerHTML = "Ore value"
		if (currentOre<79) {
			document.getElementById("currentOreValue").innerHTML = "$" + format(oreValues[currentOre-1])
		}
		else {
			document.getElementById("currentOreValue").innerHTML = "$" + format(1.8**(currentOre-78)*8e17)
		}
	}
}
...
pointerDown = 0
let holdDownTimeout
function mineDown() {pointerDown = 1; holdDownTimeout = setTimeout(holdDownMine, 200)}
function mineUp() {pointerDown = 0; clearTimeout(holdDownTimeout)}

function holdDownMine() {
	if (pointerDown) {
		mineOre()
		holdDownTimeout = setTimeout(holdDownMine, 200)
	}
}
